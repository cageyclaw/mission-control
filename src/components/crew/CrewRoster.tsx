...
export default function CrewRoster() {
  ...
  return (
    <div>
      ...
      <div className="occ-section-header occ-section-header--orange">
        <span className="occ-section-header__number">47-15</span>
      ...
      {activeCrew.map(member => (
        <div
          key={member.id}
          className={`occ-list-item ${selectedCrewId === member.id ? 'occ-list-item--selected' : ''}`}
          onClick={() => selectCrew(selectedCrewId === member.id ? null : member.id)}
        >
          {/* Reference Number */}
          <span className="occ-list-item__number">{CREW_NUMBERS[member.id] || '47-XX'}</span>
          
          {/* Status Dot */}
          <span className={`status-dot status-dot--${member.status}`} />
          
          {/* Emoji */}
          <span className="occ-list-item__emoji">{member.emoji}</span>
          
          {/* Info */}
          <div className="occ-list-item__info">
            <div className="occ-list-item__name">{member.name}</div>
            <div className="occ-list-item__role">
              {member.currentTask
